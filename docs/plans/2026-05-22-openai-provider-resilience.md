...
- [ ] update `openai.go` to resolve `providerhttp.ForProvider(cfg, "openai")`
- [ ] update `openai.go` to resolve `providerretry.ForProvider(cfg, "openai")`
- [ ] store HTTP client and retry config in provider state
- [ ] preserve existing API key, model, base URL, and request mutation behavior
